...
var popup_template = '<h2><a href="/shakha/view/{shakha_id}">{name}</a></h2><h3>{output_add}</h3><p>{frequency} every {frequency_day}</p>';

new L.GeoJSON(shakhas, {
...
// new L.GeoJSON(shakhas).addTo(map);
// var shakha_fips = shakhas.map(a => a.state_fips.padStart(2, '0') + a.county_fips.padStart(3,'0'));
