...
echo strlen(trim($name)) ? $name : 'Name Unavailable';?> <?php
...
<span class="rightcol"> <?php if(!empty($row->ph_mobile) || !empty($row->ph_home) || !empty($row->ph_work) || !empty($row->email)): ?>
...
<?php if(strlen(trim($row->position))) echo $row->position.', ';?><?php if(strlen(trim($row->company))) echo $row->company;?><br />
<br />
<?php endif; ?> <br />
</span>
