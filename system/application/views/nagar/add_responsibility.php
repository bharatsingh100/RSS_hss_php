<h2><?=$row->name;?> Nagar - Manage Responsibilities</h2>
<p>&nbsp;</p>
<?php
...
$allowed_res = array('010', '011', '020', '021', '110', '050', '051',
                     ...
                     '270', '272');
$nagar_id = $this->uri->segment(3);
//$vibhag_id = substr($this->uri->segment(3),0,4);
if(isset($row->kk))
{
	...
	foreach ($row->kk as $kk)
	{
		...
		echo anchor('nagar/del_responsibility/'.$nagar_id.'/'.$kk->contact_id.'/'.$kk->responsibility, '&nbsp;(remove)&nbsp;');}
		echo '</p>';
	}
	echo '<br />';
}
?>
           ...
           <?=form_hidden('nagar_id', $nagar_id);?>
           <?=form_hidden('level', 'NA');?>
