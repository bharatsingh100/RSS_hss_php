...
class Permission
{
    ...
    function Permission()
    {
		...
		$temp = $this->CI->db->getwhere('responsibilities', array('swayamsevak_id' => $this->cid, 'level' => 'SH'));
		...
		//IS_NT_KK
		$rs = $this->CI->db->getwhere('responsibilities', array('swayamsevak_id' => $this->cid, 'level' => 'NT'));
	...
	{ 
		...
		$rs = $this->CI->db->getwhere('responsibilities', array('swayamsevak_id' => $this->cid, 'level' => 'SA', 'sambhag_id' => $id));
	...
	function is_vibhag_kk($id)
	{
		...
		$rs = $this->CI->db->getwhere('responsibilities', array('swayamsevak_id' => $this->cid, 'level' => 'VI', 'vibhag_id' => $id));
		$this->is_vibhag_kk = ($rs->num_rows() || $this->is_sambhag_kk) ? true : false;
		return $this->is_vibhag_kk;
	}
	
	function is_shakha_kkh($id)
	{
		$rs = $this->CI->db->getwhere('shakhas', array('shakha_id' => $id))->row();
		...
		$this->is_shakha_kkh = (((in_array($this->sh_resp, $a) && $this->shakha_id == $id) ||  $this->is_vibhag_kk($rs->vibhag_id) ) ? true : false);
		return $this->is_shakha_kkh;
	}
	
	function is_shakha_kkl($id)
	{
		$a = array('040','050','051','060','061','070','071','080','081','010','011','090','091','100','101','110','120','130','131','999','140');
	...
	function allow_profile_edit($id)
	{
		...
		$rs = $this->CI->db->select('shakha_id')->getwhere('swayamsevaks', array('contact_id' => $id))->row();
		if($rs->shakha_id != '')
		{
			$ps = $this->CI->db->getwhere('shakhas', array('shakha_id' => $rs->shakha_id))->row();
